...
export class InterceptorService extends Http {
  ...
  request(url: string | Request, options?: RequestOptionsArgs): Observable<Response> {
    ...
    if (typeof url == 'string') {
      ...
      console.log(str);
      url = url.replace('achievement-web', 'http://192.168.22.202:8080/dmtGraduatorHelp-web');
    
    } else {
      const str = url.url.substring(0, url.url.indexOf('/'));
      url.url = url.url.replace('achievement-web', 'http://192.168.22.202:8080/dmtGraduatorHelp-web');
